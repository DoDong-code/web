...
export default function App() {
  ...
  const handleHeroGalleryPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const dragThreshold = drag.pointerType === 'touch' ? 14 : 5;
    if (Math.hypot(distanceX, distanceY) > dragThreshold) drag.didDrag = true;
    heroGalleryMotionRef.current.target = drag.scrollLeft - distanceX * HERO_GALLERY_SCROLL_SPEED;
  };
  ...
  const handleHeroGalleryPointerUp = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const tapThreshold = drag.pointerType === 'touch' ? 10 : 5;
    if (!drag.didDrag && releaseDistance <= tapThreshold) {
